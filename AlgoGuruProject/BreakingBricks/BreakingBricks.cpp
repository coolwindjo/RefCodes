...
{
	if (!WARN_IFNOT(rowIdx < MAX_NUM_ROWS)
		|| !WARN_IFNOT(colIdx < MAX_NUM_COLS)
		|| !WARN_IFNOT(value >= 0 && value <= 9))
	{
		return -1;
	}
...
{
	if (!WARN_IFNOT(m_W_numBrickCols > 0) || !WARN_IFNOT(m_H_numBrickRows > 0))
	{
		return -1;
	}
	...
	WARN_IFNOT(m_minRemainedNumBricks < m_inputNumBricks);
...
{
	// Basis part
	if (!WARN_IFNOT(remainedNumBricks >= 0))
	{
	...
	for (size_t colIdx = 0; colIdx < m_W_numBrickCols; colIdx++)
	{
		...
		if (numRemovedBricks > 0)
		{
			int numProcessedBricks = FallBricksAndFillEmptySpace(numRemovedBricks,
																 m_matMap[ballNum]);
			WARN_IFNOT(numProcessedBricks == numRemovedBricks);
							   ...
							   int &numRemovedBricksOut, int(&matMapOut)[MAX_NUM_ROWS][MAX_NUM_COLS])
{
	// Basis part
	int brickValue = 0;
	if (!WARN_IFNOT((brickValue = RemoveBrick(rowIdxToStart, colIdxToStart, numRemovedBricksOut, matMapOut)) >= 0))
	{
		return;
	}
...
{
	// Guard clauses
	if (!WARN_IFNOT(rowIdx < m_H_numBrickRows)
		|| !WARN_IFNOT(colIdx < m_W_numBrickCols))
	{
		return -1;
	}
