...
#if 0
			m_visit.push_back(const_cast<const vstr&>(vstrVisit));
#endif // 0
		}
		m_rows = m_map.size();
...
private:
	...
	void _BFSWithUniqueAndSmallest(const ii &start) {
		...
		m_posQ.push(start);
#if 0
		m_visit[start.first][start.second] = m_ans;
#endif // 0
		while (true) {
			while (!m_posQ.empty()) {
	...
	void _DFSWithBackTracking(const ii &pos, string &strNameOut) {
...
#if 0
		m_visit[pos.first][pos.second] = strNameOut;
#endif // 0
		...
		FOR(dir, eDIR_LEN) {
			...
			if (OOR(nextPos.second, 0, (int)(m_cols - 1))) continue;
#if 0
			if (m_visit[nextPos.first][nextPos.second].compare(strNameOut + (m_map[nextPos.first][nextPos.second])) < 0) continue;
#endif // 0
