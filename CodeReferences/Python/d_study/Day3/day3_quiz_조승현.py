...
class GraphManager():
    ...
    def build_graph(self):
        ...
        with self.graph.as_default():
            self.x_node = tf.placeholder(dtype=tf.float64, shape=[None, 3], name='X')
            self.y_node = tf.placeholder(dtype=tf.float64, shape=[None, 1], name='Y')

            # Weight, bias initialization
            tf.set_random_seed(777)
            self.weight = tf.Variable(tf.random_uniform([3, 1], seed=0, dtype=tf.float64), name='W')
            self.bias = tf.Variable(tf.random_uniform([1], seed=0, dtype=tf.float64), name='B')
            self.init = tf.global_variables_initializer()

            with tf.name_scope('inference'):
                self.y_o = tf.add(tf.matmul(self.x_node, self.weight), self.bias)
    ...
    def train_data(self, x_data, y_data):
        ...
        with self.graph.as_default():
            ...
            for step in range(100):
                ...
                if step % 5 == 0:
                    print("Epoch:{} Cost={:.04f} Weight=[{:.4f}, {:.4f}, {:.4f}] Bias={:.4f}"\
                          .format(step, _loss,
                                  _weight[0, 0], _weight[1, 0], _weight[2, 0], _bias[0]))
    ...
    def test_with_model(self, test_x, gt_y):
        ...
        with self.graph.as_default():
            ...
            print("TestSet:{} Result:{} Ground truth:{}"\
...
def main():
    ...
    with GraphManager() as grp_man:
        grp_man.train_data(x_data, y_data)
