...
class ChessTargetShape(object):
    ...
    def generate_DFLZ_target_shape_in_road_coordinate(self, wheelbase):
        try:
            homography = hm.Homography()
            ...
            target_location = hm.Translation3D((wheelbase + self.spec.Daxle), 0, self.spec.Height)
...
def main():
    ...
    try:
        ...
        chess_target.generate_DFLZ_target_shape_in_road_coordinate(wheelbase)

        homography = hm.Homography()
        H = homography.generate_3x4_homography_matrix(wheelbase_and_tire_radius, calibration_parameters)

        (   black_rects_on_image, 
            white_rects_on_image) = chess_target.compute_onto_image_using_homography(H)
        ...
        H_4x4 = homography.generate_4x4_homography_matrix(wheelbase_and_tire_radius, calibration_parameters)

        with open('GT.txt', 'w') as fileout:
            for point in chess_target.detectable_points:
                ground_truth_point = homography.compute_3d_to_2d_with_3x4_homography(H, point)
