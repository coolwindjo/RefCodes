...
class FileInput(object):
    ...
    def read_calibration_parameters_from_xml(self, filename):
        ...
        try:
            ...
            camera_loc = hm.Point3D(float(parameter_dict["fCameraMountPose_x_factory"]),
                                    float(parameter_dict["fCameraMountPose_y_factory"]), 
                                    float(parameter_dict["fCameraMountPose_z_factory"]))
            camera_rot = hm.Rotation3D( float(parameter_dict["fCameraMountPose_roll_factory"]), 
                                        float(parameter_dict["fCameraMountPose_pitch_factory"]), 
                                        float(parameter_dict["fCameraMountPose_yaw_factory"]))
            camera_int = hm.Intrinsic(   F_u=float(parameter_dict["fFocallengthU_factory"]), 
                                        F_v=float(parameter_dict["fFocallengthV_factory"]), 
                                        C_u=float(parameter_dict["fOpticalCenterU_factory"]),
                                        C_v=float(parameter_dict["fOpticalCenterV_factory"]))
    ...
    def read_wheelbase_and_tire_radius_from_xml(self, filename):
        ...
        try:
            ...
            out_dict_values = {
                'wheelbase'     : float(parameter_dict["fWheelbase"]),
                'tire_radius'   : float(parameter_dict["fTireRadius"]),
            }

        except :
            print("[Error] Unexpected error at read_wheelbase_and_tire_radius_from_xml().", sys.exc_info()[0])
            print("len(parameter_dict) = " + str(len(parameter_dict)))
                    
        return out_dict_values
...
def main():
    ...
    nmatValues1 = get_vehicle_signal_matrix_with_given_range(   strBaseFile, strShiftingFile,
